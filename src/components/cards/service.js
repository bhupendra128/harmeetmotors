  ...
  return (
    <Box sx={styles.serviceItem}>
      ...
      <Box sx={styles.content}>
        ...
        {item?.subDescriptions?.map((subItem, index) => (
          <div key={index} style={styles.subContent}>
            <Text as="p">{subItem.line1}</Text>
            <Text as="p">{subItem.line2}</Text>
            <Text as="p">{subItem.line3}</Text>
          </div>
        ))}
